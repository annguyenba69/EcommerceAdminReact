...
import { addPostCat, confirmDeletePostCat, getAllListPostCat, getDetailPostCatById } from '../../../redux/reducers/PostCatReducer';
import AdminEditPostCat from './AdminEditPostCat';
import { setVisibleTrueAction } from '../../../redux/reducers/DrawerReducer';
const { Search } = Input;
const { Option } = Select;
export default function AdminListPostCat() {
                    ...
                    <Tooltip title="Edit">
                        <Button onClick={async () => {
                            let payload = {
                                ...
                                Component: <AdminEditPostCat></AdminEditPostCat>,
                            }
                            await dispatch(getDetailPostCatById(record.id));
                            await dispatch(setVisibleTrueAction(payload));
                        }} type="primary" shape="circle" icon={<EditOutlined />} size="large" />
                    </Tooltip>
                    <Tooltip title="Delete">
                        <Button onClick={() => {
                            dispatch(confirmDeletePostCat(record.id))
    ...
    const renderOptions = () => {
        return allListPostCats?.map((postCat, index) => {
            return <Option key={index + 1} value={postCat.id}>{"---/".repeat(postCat.level) + postCat.name}</Option>
        })
    }
    ...
    const handleSearch = (e) => {
        ...
        searchRef.current = setTimeout(() => {
            dispatch(getAllListPostCat(e.target.value));
        }, 300)
    }
    ...
    return (
        <div className='row'>
            <div className='col-md-3'>
                <div className="card">
                    <div className="card-header font-weight-bold d-flex  align-items-center">
                        <h5 className="m-0 ">Add Post Categories</h5>
                    </div>
            ...
            <div className='col-md-9'>
                <div className="card">
                    <div className="card-header font-weight-bold d-flex  align-items-center">
                        <h5 className="m-0 ">List Post Categories</h5>
                    </div>
                    <div className="card-body">
                        ...
                        <>
                            <Table columns={columns} dataSource={allListPostCats} onChange={handleChange} pagination={{
                                onChange(current) {
                                    setPage(current);
                                },
                                pageSize: 6,
